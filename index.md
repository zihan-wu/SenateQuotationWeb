...
In general, more quotations in news articles indicate more popularity or at least more exposure to the public. Quotations could thereby be vital for the successful careers of politicians. On the other hand, successful politicians are also more frequently quoted in news. **Thus, we think that the career of politicians may be highly correlated with quotations.** The recent release of QuoteBank, a corpus of quotations from news, can help us extract the quotations of politicians and explore their correlation with election results!

Let's focus on the U.S. Senate elections. By combining the quotations and election results, we can first answer the question of whether more quoted senate election candidates tend to have better election results. Then, we may examine if quotations can be an efficient predictor of election results. Furthermore, quotations can either praise or criticize the politicians. Recent advancements in language sentiment analysis enable us to get attitudes toward quoted candidates. We will also explore how such attitudes correlated with election results.

## Extract candidate quotes:

To begin our journey, we obtain the senate election result from MIT Election Data and Science Lab[1]. It contains the election information of each candidate, including identity, party affiliation, and resulting votes. We select the candidates participating in **U.S. Senate elections since 2016**. In QuoteBank corpus, we find quotations **said by** these candidates in news articles **from 2015 to 2020**. By aggregating the quotations based on candidates and dates, we look at how the number of quotations said by a selected candidate changes over time. Let's look at an example from Bernie Sanders:

<p align="center">
  <img src="figures/Sanders_said.png" />
</p>

A lot of quotations in 2016 and 2020, possibly because he was competing for the Democratic Party's nomination for presidential elections.

On the other hand, there is another type of quotation: quotations **mentioning the candidates** but not necessarily said by themselves. In QuoteBank corpus, we can also find such quotations mentioning the Senate election candidates in news articles from 2015 to 2020. By aggregating the quotations based on candidates and dates again, we can look at how the number of quotations mentioning a selected candidate changes over time. Let's look at Bernie Sanders from this perspective:

<p align="center">
  <img src="figures/Sanders_mention.png" />
</p>

(Note: QuoteBank may contain different numbers of quotations each month. So, we normalize each month's quotations by the monthly total quotes contained in Quotebank. Detail can be found in the jupyter notebook in the GitHub repository.)

## Total quotation is positively correlated to results.

Let's now calculate each candidate's total quotations before the election and compare the number of total quotations to election results. For both types of quotations, we use scatter plots to show how total quotation numbers and vote rates are related.
...
For each type, let's look at one example candidate:
...
Interestingly, politicians in type 1 appear to have significantly **higher vote rate** (t-test, p-value < 0.01). The major quotation peak right before the election may indicate that the last month is crucial in the election campaign. If I were the campaign manager of a senate candidate, I would advise him or her to try to get his or her quotations on major news articles!

## Can quotations predict election result?

So far, it appears that quotations are indeed related to election results. Naturally, one can ask the question: can the election results be predicted from the quotation data? We use the **monthly aggregated quotation feature vectors** used in the clustering analysis above. Besides, party affiliation can have a significant effect on the result. For senate candidates in each state's election, we consider the **vote rate of their affiliated party** in the previous presidential election[2]. This party vote rate reflects the overall ideology of the state. We concatenate this voting feature to the quotation feature vectors and use this 11-dimensional vector for predicting the election result.
...
To examine whether sentiments in quotations can represent support rate, we can calculate the **average sentiment score of quotations** mentioning each candidate **within a year before the election**. We discard candidates having a low vote_rate (< 1%), because they are often representatives of small parties and their quotation data are too small to mirror the overall sentiments. The relation can be visualized by scatter plots of average sentiment score and final vote rate:

<p align="center">
  <img src="figures/senti_vs_vote.png" />
</p>

The Pearson Correlation is -0.11, with a P-Value 0.03 (< 0.05), which is statistically significant. **Sentiments in the media falsely reflect the public's opinions!** Such observation seems to be contrary to the intuition that more favorable people will receive more votes. We propose three possible explanations for this: 
1. The dataset of quotes mentioning candidates is **not large enough** and the correlation here is spurious. 
